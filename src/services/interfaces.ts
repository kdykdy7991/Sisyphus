...
export interface KnowledgeService{list():Promise<Knowledge[]>;listTopics():Promise<string[]>;createTopic(name:string):Promise<void>;get(id:string):Promise<Knowledge|undefined>;search(query:string):Promise<Knowledge[]>;save(item:Knowledge):Promise<Knowledge>;recent(limit?:number):Promise<Knowledge[]>;delete(id:string):Promise<boolean>;clear():Promise<number>;exportMarkdown(suggestedName:string,ids?:string[]):Promise<KnowledgeExportSummary|null>}
