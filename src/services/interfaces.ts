import type {BackupInspect,BackupRestoreResult,BackupSummary,ChatMessage,ConnectionResult,ImportImage,Knowledge,KnowledgeExportSummary,LlmProfiles,Settings,SimilaritySuggestion,SyncExportSummary,SyncImportSummary,SyncInspectReport,WebDavConfig,WebDavSyncSummary,WebDavTestResult} from '../types';
export interface KnowledgeService{list():Promise<Knowledge[]>;listTopics():Promise<string[]>;createTopic(name:string):Promise<void>;deleteTopic(name:string):Promise<boolean>;get(id:string):Promise<Knowledge|undefined>;search(query:string):Promise<Knowledge[]>;save(item:Knowledge):Promise<Knowledge>;recent(limit?:number):Promise<Knowledge[]>;delete(id:string):Promise<boolean>;clear():Promise<number>;exportMarkdown(suggestedName:string,ids?:string[]):Promise<KnowledgeExportSummary|null>}
export interface ImportService{extract(images:ImportImage[],onProgress:(images:ImportImage[])=>void):Promise<ImportImage[]>;extractUrl(url:string):Promise<Knowledge[]>;confirm(items:Knowledge[]):Promise<void>;suggestSimilarity(draft:Knowledge):Promise<SimilaritySuggestion|null>;updateExisting(draft:Knowledge,existingId:string):Promise<void>}
export interface ChatService{ask(question:string,scope:string,history:ChatMessage[]):Promise<ChatMessage>}
export interface SettingsService{get():Promise<Settings>;save(value:Settings):Promise<void>;profiles():Promise<LlmProfiles>;createProfile(name:string):Promise<string>;renameProfile(id:string,name:string):Promise<void>;switchProfile(id:string):Promise<Settings>;deleteProfile(id:string):Promise<void>;testConnection():Promise<ConnectionResult>}
