...
- blockquote 的正式语义是 Important / 重点提醒。只有当一段内容是该题最需要记住的一句话、最容易混淆或答错的点、理解整题的关键结论，或明显的注意事项时，才可以使用 blockquote。
- 例如：`> MVCC 的核心目标不是消除所有并发冲突，而是让读写尽可能互不阻塞。`
- 不要为了“好看”生成 blockquote，不要把普通知识点、普通定义、普通列表项或普通总结机械转换成 blockquote。
- 每个 answer 通常使用 0~1 个 blockquote；复杂答案最多 2 个。禁止整篇大量使用 blockquote。
